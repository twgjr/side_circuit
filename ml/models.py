...
class Solver(nn.Module):
    ...
    def __init__(self, data: Data):
        ...
        self.i_base = self.data.base(self.data.prop_list(Props.I,True))
    ...
    def init_attr(self):
        ...
        ret_tensor = torch.tensor(
            self.data.normalize(self.i_base,ics_list)).to(torch.float)
        ivs_list = self.data.attr_list(Kinds.IVS)
        ivs = torch.tensor(
            self.data.normalize(self.v_base,ivs_list)).to(torch.float)
        ret_tensor[self.ivs_mask] = ivs[self.ivs_mask]
        r_list = self.data.attr_list(Kinds.R)
        r = torch.tensor(
            self.data.normalize(self.r_base,r_list)).to(torch.float)
